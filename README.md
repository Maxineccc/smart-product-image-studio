...
当前版本 v2.9.0：单张图片与批量 ZIP 内图片统一命名为 `AI模板-YYYYMMDD-品牌名称-模板名称-CST-动态标号.jpg`。流水号按“日期 + 品牌”保存在浏览器本地，同一品牌当天从 01 顺延；换品牌或换日期后从 01 开始。ZIP 文件名同时标出本批次起止编号。

模板按 800×800 方形和 1280×720 横版分组，共十套，可全选、取消全选或单独勾选；ZIP 只生成已选模板。全部模板统一使用用户提供的淘宝闪购透明 Logo，并绘制用户选择的警示语；警示语默认位于底部，可调整左右与上下位置。
