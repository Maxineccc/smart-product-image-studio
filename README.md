...
当前版本 v2.6.0：主流程简化为“上传豆包处理好的商品图 → 填写商品信息 → 设置全模板警示语 → 按尺寸勾选模板 → 批量导出”。主体大小、位置、旋转、透明度、投影和翻转改为按模板分别保存，只调整当前选中模板，批量导出时分别应用。结算单文案改为“加群可再领2元”并移除配送费划线原价；红包券商品下移，双杯模板实付区上移。简洁好价横图的警示语默认放在左侧橙色区域。
