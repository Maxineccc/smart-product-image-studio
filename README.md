...
当前版本 v2.5.2：主流程简化为“上传豆包处理好的商品图 → 填写商品信息 → 设置全模板警示语 → 按尺寸勾选模板 → 批量导出”。默认品牌和产品为“瑞幸咖啡 / 标准美式”。留白单品模板不再显示商品名称，红包券模板已缩小商品避免压住文案；新增一套 800×800 方形爆款咖啡模板，爆款标题可自定义。警示语下拉框已限制在设置卡片内，超长内容不再溢出；警示语只绘制文字并直接融入模板背景。上传区域改为说明、双按钮和话术入口分层排列。
