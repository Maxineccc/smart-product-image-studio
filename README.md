...
当前版本 v2.3.1：主流程简化为“上传豆包处理好的商品图 → 填写商品信息 → 按尺寸勾选模板 → 批量导出”。原截图 OCR、框选和浏览器抠图模型入口已从界面移除；所有模板直接绘制透明淘宝闪购 Logo，不再添加白色圆角底板。

模板按 800×800 方形和 1280×720 横版分组，可全选、取消全选或单独勾选；ZIP 只生成已选模板。全部模板统一使用用户提供的淘宝闪购透明 Logo。结算单模板采用较小的主体默认尺寸，纯色底抠图包含边缘去色溢处理。
