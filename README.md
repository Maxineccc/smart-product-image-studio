...
当前版本 v2.1：升级为双模板商品图工作台，并新增“豆包纯色底模式”。网站提供纯品红色 `#FF00FF` 专用话术，接受豆包生成的高饱和均匀纯色背景商品图，再通过边缘连通色键自动转换成真实透明图层；白底、棋盘格、渐变或低饱和背景会被拒绝。真正透明的 PNG/WebP 仍可作为原图图层直接载入，不经过抠图、锐化或色彩修改。
