...
当前版本 v2.4.0：主流程简化为“上传豆包处理好的商品图 → 填写商品信息 → 按尺寸勾选模板 → 批量导出”。原截图 OCR、框选和浏览器抠图模型入口已从界面移除；所有模板直接绘制透明淘宝闪购 Logo。已删除“橙红爆款”和“清新推荐”，新增朋友圈红包券、留白单品、左右好价、双杯下单、场景种草五套淘宝闪购参考版式。

模板按 800×800 方形和 1280×720 横版分组，共九套，可全选、取消全选或单独勾选；ZIP 只生成已选模板。全部模板统一使用用户提供的淘宝闪购透明 Logo。结算单模板的左上角缩略图采用完整显示与适量留白，纯色底抠图包含边缘去色溢处理。
