...
当前版本 v3.0.0：新增全冰去冰、人气 TOP、爆款回归三套 800×800 淘宝闪购方图。三套模板均采用浅蓝圆点视觉，左上角绘制淘宝闪购 Logo，仅使用上传的商品主体，不加入黄油、椰子等道具；底部使用当前到手价并显示“元起”。模板带实时缩略图、独立主体微调和统一动态编号。

模板按 800×800 方形和 1280×720 横版分组，共十三套，可全选、取消全选或单独勾选；ZIP 只生成已选模板。全部模板统一使用用户提供的淘宝闪购透明 Logo，并绘制用户选择的警示语；警示语默认位于底部，可调整左右与上下位置。
