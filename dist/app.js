...
function renderMilkDaily(){const v=vals(),headline=(v.adHeadline||'奶茶天天喝').trim(),product=(v.productName||'产品名称').trim(),g=ctx.createLinearGradient(0,0,0,800);g.addColorStop(0,'#dff5ff');g.addColorStop(.62,'#b8e7fb');g.addColorStop(.63,'#fffdec');g.addColorStop(1,'#fff9dc');ctx.fillStyle=g;ctx.fillRect(0,0,800,800);ctx.save();ctx.fillStyle='rgba(255,255,255,.76)';[[690,150,56],[92,390,18],[360,310,12],[720,420,15],[190,640,28]].forEach(([x,y,r])=>{ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()});ctx.restore();ctx.strokeStyle='rgba(255,255,255,.8)';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(0,564);ctx.quadraticCurveTo(400,525,800,575);ctx.stroke();drawLogo(38,30,178);const hs=headline.length>8?42:52;text(headline,48,160,hs,'900','#0a2450');text(product,50,222,28,'700','#17365f');drawSubject(475+getPlacement().x,625+getPlacement().y,270,430,'上传商品主体');drawSubject(650+getPlacement().x,630+getPlacement().y,235,390,'上传商品主体');fillRound(245,660,330,86,43,'#ff5a1f');text('券后',290,703,21,'750','#fff','center');text('¥'+money(v.finalPrice),423,702,49,'900','#fff','center');text('起',538,706,22,'750','#fff','center')}
function renderGreenFresh(){const v=vals(),headline=(v.productName||'清爽果茶').trim(),sub=(v.adHeadline||'清爽果香 一口入夏').trim(),g=ctx.createLinearGradient(0,0,0,720);g.addColorStop(0,'#fff');g.addColorStop(.58,'#f7fff0');g.addColorStop(1,'#baff74');ctx.fillStyle=g;ctx.fillRect(0,0,1280,720);ctx.save();ctx.strokeStyle='rgba(91,176,35,.42)';ctx.lineWidth=3;for(let i=0;i<4;i++){ctx.beginPath();ctx.ellipse(480,410,470+i*32,170+i*14,-.08,0,Math.PI*2);ctx.stroke()}ctx.fillStyle='rgba(255,255,255,.75)';[[620,110,18],[1110,200,11],[1145,375,16],[360,610,13],[1010,630,20]].forEach(([x,y,r])=>{ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()});ctx.restore();drawLogo(48,34,205);text(headline,58,278,headline.length>9?50:64,'900','#123f16');text(sub,62,352,34,'650','#285b2b');text('新人价',65,470,24,'750','#243324');text('¥',62,548,38,'700','#f25a13');text(money(v.finalPrice),105,535,82,'900','#f25a13');text('起',350,550,28,'750','#1e2a1e');fillRound(57,588,300,68,34,'#ff5a1f');text('立即抢购',207,622,30,'850','#fff','center');drawSubject(875+getPlacement().x,700+getPlacement().y,500,625,'上传商品主体')}
function renderDarkMascarpone(){const v=vals(),product=(v.productName||'空气马斯卡彭').trim(),sub=(v.adHeadline||'口感丰富 入口清甜甘醇').trim(),cut=product.length>6?Math.ceil(product.length/2):product.length,g=ctx.createRadialGradient(610,350,40,610,350,760);g.addColorStop(0,'#263025');g.addColorStop(.5,'#101711');g.addColorStop(1,'#020504');ctx.fillStyle=g;ctx.fillRect(0,0,800,800);ctx.save();ctx.strokeStyle='rgba(255,255,255,.025)';ctx.lineWidth=2;for(let i=-200;i<1000;i+=18){ctx.beginPath();ctx.moveTo(i,0);ctx.lineTo(i-260,800);ctx.stroke()}ctx.restore();drawLogo(38,31,180);text(product.slice(0,cut),48,190,product.length>10?39:48,'900','#fff');if(product.length>6)text(product.slice(cut),48,252,product.length>10?39:48,'900','#fff');text(sub,50,330,24,'650','#f3f1e7');text('新人专享价',50,430,24,'700','#f1d6a6');text('¥',50,526,38,'700','#fff');text(money(v.finalPrice),92,505,100,'900','#fff');text('起',335,527,30,'800','#fff');fillRound(50,585,285,68,34,'#f25a1b');text('立即领取',193,619,30,'850','#fff','center');drawSubject(590+getPlacement().x,735+getPlacement().y,390,620,'上传商品主体')}
function renderStreetDeal(){const v=vals(),product=(v.productName||'招牌奶茶').trim(),sub=(v.adHeadline||'闪购专属福利价').trim(),g=ctx.createLinearGradient(0,0,0,720);g.addColorStop(0,'#90cdf8');g.addColorStop(.45,'#8fbd75');g.addColorStop(.46,'#5f705d');g.addColorStop(1,'#7e817a');ctx.fillStyle=g;ctx.fillRect(0,0,1280,720);ctx.save();ctx.filter='blur(24px)';ctx.fillStyle='#2b6e2c';[[80,150,185],[300,90,150],[560,125,170],[1060,110,220],[1240,250,180]].forEach(([x,y,r])=>{ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()});ctx.fillStyle='rgba(255,255,255,.52)';ctx.fillRect(0,280,1280,34);ctx.restore();const shade=ctx.createLinearGradient(0,0,730,0);shade.addColorStop(0,'rgba(6,18,17,.75)');shade.addColorStop(.75,'rgba(6,18,17,.16)');shade.addColorStop(1,'rgba(6,18,17,0)');ctx.fillStyle=shade;ctx.fillRect(0,0,760,720);drawLogo(48,34,205);text(product,58,260,product.length>8?50:68,'900','#fff');text(sub,60,340,42,'800','#fff');text('到手价',62,455,26,'750','#fff');text('¥',58,535,34,'750','#fff');text(money(v.finalPrice),98,520,74,'900','#fff');text('起',322,536,28,'750','#fff');fillRound(55,575,318,72,36,'#fff');text('立即抢购  ›',214,611,31,'850','#17251c','center');drawSubject(950+getPlacement().x,710+getPlacement().y,470,640,'上传商品主体')}
...
function render(skipThumbs=false){const def=templateDefs[state.template]||templateDefs.square,w=def.w,h=def.h;if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;canvas.style.aspectRatio=`${w}/${h}`}ctx.clearRect(0,0,w,h);({square:renderSquare,orderDetail:renderOrderDetail,socialCoupon:renderSocialCoupon,socialMinimal:renderSocialMinimal,socialDeal:renderSocialDeal,socialDuo:renderSocialDuo,socialScene:renderSocialScene,squareBestseller:renderSquareBestseller,blueIce:renderBlueIce,blueTop:renderBlueTop,blueReturn:renderBlueReturn,milkDaily:renderMilkDaily,darkMascarpone:renderDarkMascarpone,cafe:renderCafe,greenFresh:renderGreenFresh,streetDeal:renderStreetDeal,landscapeClean:renderLandscapeClean,landscapeDark:renderLandscapeDark}[state.template]||renderSquare)();drawWarning();if(!skipThumbs)scheduleTemplateThumbs()}
...
function removeLowerContactShadow(src){
  const c=src.getContext('2d',{willReadFrequently:true}),im=c.getImageData(0,0,src.width,src.height),d=im.data,w=src.width,h=src.height,counts=new Int32Array(h);let y0=h,y1=0;
  for(let y=0;y<h;y++){let n=0;for(let x=0;x<w;x++)if(d[(y*w+x)*4+3]>30)n++;counts[y]=n;if(n){y0=Math.min(y0,y);y1=Math.max(y1,y)}}
  if(y1<=y0)return src;const bh=y1-y0+1,body=[];
  for(let y=Math.round(y0+bh*.42);y<=Math.round(y0+bh*.68);y++)if(counts[y])body.push(counts[y]);
  if(!body.length)return src;body.sort((a,b)=>a-b);const bodyWidth=body[body.length>>1],thin=Math.max(3,Math.round(bodyWidth*.18)),shadowWide=Math.max(8,Math.round(bodyWidth*.28));let cut=-1;
  for(let y=Math.round(y0+bh*.64);y<y1-2;y++){
    if(counts[y]>thin||counts[y+1]>thin)continue;let belowMax=0,belowStart=h,belowEnd=0,belowPixels=0;
    for(let yy=y+2;yy<=y1;yy++)if(counts[yy]){belowMax=Math.max(belowMax,counts[yy]);belowStart=Math.min(belowStart,yy);belowEnd=Math.max(belowEnd,yy);belowPixels+=counts[yy]}
    const belowHeight=belowEnd-belowStart+1;
    if(belowMax>=shadowWide&&belowPixels>=bodyWidth*1.5&&belowHeight>0&&belowHeight<=bh*.2){cut=y;break}
  }
  if(cut<0)return src;for(let y=cut;y<h;y++)for(let x=0;x<w;x++)d[(y*w+x)*4+3]=0;c.putImageData(im,0,0);return trimCanvas(src);
}
function removeBottomChromaShadow(src,bg){
  const c=src.getContext('2d',{willReadFrequently:true}),im=c.getImageData(0,0,src.width,src.height),d=im.data,w=src.width,h=src.height;let ax0=w,ay0=h,ax1=0,ay1=0;
  for(let y=0;y<h;y++)for(let x=0;x<w;x++)if(d[(y*w+x)*4+3]>20){ax0=Math.min(ax0,x);ay0=Math.min(ay0,y);ax1=Math.max(ax1,x);ay1=Math.max(ay1,y)}
  if(ax1<=ax0||ay1<=ay0)return src;const hue=(r,g,b)=>{r/=255;g/=255;b/=255;const hi=Math.max(r,g,b),lo=Math.min(r,g,b),v=hi-lo;if(!v)return 0;let h=hi===r?(g-b)/v+(g<b?6:0):hi===g?(b-r)/v+2:(r-g)/v+4;return h*60},bgHue=hue(...bg),bh=ay1-ay0+1,bw=ax1-ax0+1,startY=Math.round(ay0+bh*.62),candidate=new Uint8Array(w*h),seen=new Uint8Array(w*h);
  for(let y=startY;y<=ay1;y++)for(let x=ax0;x<=ax1;x++){const p=y*w+x,k=p*4,r=d[k],g=d[k+1],b=d[k+2],hi=Math.max(r,g,b),lo=Math.min(r,g,b),sat=hi?(hi-lo)/hi:0,hd=Math.abs(hue(r,g,b)-bgHue),hueDist=Math.min(hd,360-hd);if(d[k+3]>15&&sat>.2&&hueDist<34)candidate[p]=1}
  for(let start=startY*w;start<w*h;start++){
    if(!candidate[start]||seen[start])continue;const q=[start];seen[start]=1;let x0=w,y0=h,x1=0,y1=0,n=0;
    for(let i=0;i<q.length;i++){const p=q[i],x=p%w,y=(p/w)|0;n++;x0=Math.min(x0,x);y0=Math.min(y0,y);x1=Math.max(x1,x);y1=Math.max(y1,y);for(const ni of [x?p-1:-1,x<w-1?p+1:-1,y?p-w:-1,y<h-1?p+w:-1])if(ni>=0&&candidate[ni]&&!seen[ni]){seen[ni]=1;q.push(ni)}}
    const cw=x1-x0+1,ch=y1-y0+1,flat=cw>ch*1.15&&cw>bw*.18,atBottom=y1>=ay1-3,low=y0>ay0+bh*.66;if(flat&&atBottom&&low&&n>cw*1.2)for(const p of q)d[p*4+3]=0;
  }
  c.putImageData(im,0,0);return trimCanvas(src);
}
...
const templateFileLabels={square:'结算单',orderDetail:'订单费用明细',socialCoupon:'红包券卡',socialMinimal:'留白单品',socialDeal:'左右好价',socialDuo:'双杯下单',socialScene:'场景种草',squareBestseller:'爆款咖啡',blueIce:'全冰去冰',blueTop:'人气TOP',blueReturn:'爆款回归',milkDaily:'奶茶日常',darkMascarpone:'黑金新品',cafe:'咖啡场景',greenFresh:'清爽果茶',streetDeal:'街景福利',landscapeClean:'简洁好价',landscapeDark:'质感上新'},sequenceFallback=new Map();
