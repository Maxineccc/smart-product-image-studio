...
$('#openDoubaoBtn').addEventListener('click',async()=>{if(!state.source||!state.rawCrop)return toast('请先上传商品截图并确认橙色商品框');const tab=window.open('about:blank','_blank');if(tab){tab.opener=null;tab.location.href='https://www.doubao.com/chat/'}let copied=false;try{const blob=await canvasToBlob(state.rawCrop);await navigator.clipboard.write([new ClipboardItem({'image/png':blob})]);copied=true}catch(err){console.warn('复制参考图失败',err);try{const blob=await canvasToBlob(state.rawCrop),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='豆包参考图.png';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),2000)}catch{}}$('#greenStatus').textContent=copied?'参考图已复制 · 到豆包按 Ctrl+V，再回来点②复制提示词':'浏览器未允许复制 · 已下载参考图，请上传到豆包';toast(copied?'参考图已复制，去豆包按 Ctrl+V':'参考图已下载，请上传到豆包');if(!tab)toast('请允许浏览器打开豆包窗口')});
$('#copyGreenPrompt').addEventListener('click',async()=>{let copied=true;try{await navigator.clipboard.writeText(doubaoGreenPrompt)}catch{try{const t=document.createElement('textarea');t.value=doubaoGreenPrompt;t.style.position='fixed';t.style.opacity='0';document.body.appendChild(t);t.select();copied=document.execCommand('copy');t.remove()}catch{copied=false}}if(!copied)window.prompt('请复制这段提示词后粘贴到豆包：',doubaoGreenPrompt);$('#greenStatus').textContent=copied?'提示词已复制 · 回豆包按 Ctrl+V 后开始生成':'提示词已显示，请复制后粘贴到豆包';toast(copied?'提示词已复制':'请复制显示的提示词')});
...
function removeGreenScreen(img){
  const out=document.createElement('canvas');out.width=img.naturalWidth||img.width;out.height=img.naturalHeight||img.height;const c=out.getContext('2d',{willReadFrequently:true});c.drawImage(img,0,0,out.width,out.height);const im=c.getImageData(0,0,out.width,out.height),d=im.data,w=out.width,h=out.height;
  const border=[];for(let x=0;x<w;x+=Math.max(1,Math.round(w/160))){for(const y of [0,Math.min(h-1,2),Math.max(0,h-3),h-1]){const i=(y*w+x)*4;border.push([d[i],d[i+1],d[i+2]])}}for(let y=0;y<h;y+=Math.max(1,Math.round(h/160))){for(const x of [0,Math.min(w-1,2),Math.max(0,w-3),w-1]){const i=(y*w+x)*4;border.push([d[i],d[i+1],d[i+2]])}}const median=k=>{const a=border.map(v=>v[k]).sort((a,b)=>a-b);return a[(a.length/2)|0]},bg=[median(0),median(1),median(2)],greenish=bg[1]-bg[0]>7&&bg[2]-bg[0]>3;
  const delta=(i)=>Math.max(Math.abs(d[i]-bg[0])*1.15,Math.abs(d[i+1]-bg[1]),Math.abs(d[i+2]-bg[2])),borderDelta=border.map(v=>Math.max(Math.abs(v[0]-bg[0])*1.15,Math.abs(v[1]-bg[1]),Math.abs(v[2]-bg[2]))).sort((a,b)=>a-b),tol=Math.max(24,Math.min(58,borderDelta[Math.floor(borderDelta.length*.9)]+15)),seen=new Uint8Array(w*h),queue=[];
  const keyable=p=>{const i=p*4,r=d[i],g=d[i+1],b=d[i+2],pure=g-Math.max(r,b)>18&&g>80;return pure||(greenish&&delta(i)<=tol)};const seed=p=>{if(!seen[p]&&keyable(p)){seen[p]=1;queue.push(p)}};for(let x=0;x<w;x++){seed(x);seed((h-1)*w+x)}for(let y=1;y<h-1;y++){seed(y*w);seed(y*w+w-1)}for(let q=0;q<queue.length;q++){const p=queue[q],x=p%w,y=(p/w)|0;d[p*4+3]=0;if(x>0)seed(p-1);if(x<w-1)seed(p+1);if(y>0)seed(p-w);if(y<h-1)seed(p+w)}
  for(let p=0;p<w*h;p++){if(seen[p])continue;const x=p%w,y=(p/w)|0,edge=(x&&seen[p-1])||(x<w-1&&seen[p+1])||(y&&seen[p-w])||(y<h-1&&seen[p+w]);if(!edge)continue;const i=p*4,dd=delta(i);if(greenish&&dd<tol+32)d[i+3]=Math.round(d[i+3]*Math.max(0,Math.min(1,(dd-tol+5)/37)));const r=d[i],g=d[i+1],b=d[i+2];if(g>Math.max(r,b)+4)d[i+1]=Math.min(g,Math.max(r,b)+4)}
  c.putImageData(im,0,0);return keepPrimaryProduct(out);
}
