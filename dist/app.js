...
function renderBluePromo(line1,line2){const v=vals();ctx.fillStyle='#dceeff';ctx.fillRect(0,0,800,800);ctx.save();ctx.fillStyle='rgba(255,252,220,.72)';[[90,135,30],[255,74,25],[410,160,29],[677,112,31],[735,270,23],[612,390,28],[112,420,24],[380,560,25],[700,620,31],[210,680,22]].forEach(([x,y,r])=>{ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()});ctx.restore();ctx.fillStyle='#fff';ctx.beginPath();ctx.moveTo(0,655);ctx.quadraticCurveTo(390,620,800,660);ctx.lineTo(800,800);ctx.lineTo(0,800);ctx.closePath();ctx.fill();ctx.strokeStyle='#153887';ctx.lineWidth=8;ctx.beginPath();ctx.moveTo(0,654);ctx.quadraticCurveTo(390,619,800,659);ctx.stroke();drawLogo(40,35,180);ctx.save();ctx.strokeStyle='#fff';ctx.lineWidth=12;ctx.lineJoin='round';ctx.font='900 86px "Microsoft YaHei","PingFang SC",sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.strokeText(line1,592,246);ctx.strokeText(line2,592,344);ctx.fillStyle='#173486';ctx.fillText(line1,592,246);ctx.fillText(line2,592,344);ctx.restore();drawSubject(280+getPlacement().x,700+getPlacement().y,370,540,'上传商品主体');fillRound(485,520,250,76,38,'#ff5a1f');text(money(v.finalPrice)+'元起',610,558,35,'900','#fff','center')}
function renderBlueIce(){renderBluePromo('全冰','去冰')}
function renderBlueTop(){renderBluePromo('人气','TOP')}
function renderBlueReturn(){renderBluePromo('爆款','回归')}
...
const templateFileLabels={square:'结算单',socialCoupon:'红包券卡',socialMinimal:'留白单品',socialDeal:'左右好价',socialDuo:'双杯下单',socialScene:'场景种草',squareBestseller:'爆款咖啡',blueIce:'全冰去冰',blueTop:'人气TOP',blueReturn:'爆款回归',cafe:'咖啡场景',landscapeClean:'简洁好价',landscapeDark:'质感上新'},sequenceFallback=new Map();
