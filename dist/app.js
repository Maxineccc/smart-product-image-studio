...
async function downloadAll(){const btn=$('#downloadAllBtn');if(state.bundleUrl){const a=document.createElement('a');a.href=state.bundleUrl;a.download=state.bundleName;a.click();setTimeout(()=>URL.revokeObjectURL(state.bundleUrl),2500);state.bundleUrl='';state.bundleName='';btn.textContent='批量下载全部模板';toast('六套模板 ZIP 已下载');return}if(!state.cutout)return toast('请先上传或生成商品主体');if(!window.JSZip)return toast('批量打包组件尚未加载，请稍后重试');const current=state.template,select=$('#templateSelect'),zip=new JSZip(),brand=($('#brandName').value||'品牌').replace(/[\\/:*?"<>|]/g,'_'),product=($('#productName').value||'商品').replace(/[\\/:*?"<>|]/g,'_');btn.disabled=true;try{let index=0;for(const [key,def] of Object.entries(templateDefs)){state.template=key;render();const blob=await optimizedCanvasBlob();zip.file(`${String(++index).padStart(2,'0')}-${def.file}-${def.w}x${def.h}.jpg`,blob);btn.textContent=`正在生成 ${index}/${Object.keys(templateDefs).length}`}btn.textContent='正在打包…';const bundle=await zip.generateAsync({type:'blob'});state.bundleUrl=URL.createObjectURL(bundle);state.bundleName=`${brand}-${product}-六套闪购模板.zip`;btn.textContent='ZIP已生成，点击下载';toast('六套模板已生成，请再次点击下载')}catch(e){console.warn(e);toast('批量生成失败，请重试')}finally{state.template=current;select.value=current;render();btn.disabled=false;if(!state.bundleUrl)btn.textContent='批量下载全部模板'}}
