...
const state={source:null,sourceUrl:'',crop:{x:0,y:0,w:1,h:1},nameCrop:{x:.35,y:.05,w:.55,h:.28},thumb:null,cutout:null,rawCrop:null,drag:null,exportBytes:0,batch:[],current:-1,upscaler:null,batchBusy:false};
...
function clearCurrent(){
  state.current=-1;state.source=null;state.sourceUrl='';state.thumb=null;state.cutout=null;state.rawCrop=null;
...
async function aiEnhanceCurrent(){if(!state.rawCrop||!window.Upscaler||!window.DefaultUpscalerJSModel)return false;$('#parseStatus').textContent='AI超分辨率增强中…';try{state.upscaler ||= new Upscaler({model:DefaultUpscalerJSModel});const src=await state.upscaler.upscale(state.rawCrop,{patchSize:64,padding:4});const img=new Image();await new Promise((ok,bad)=>{img.onload=ok;img.onerror=bad;img.src=src});const cv=document.createElement('canvas'),scale=Math.max(1,Math.min(4,1100/Math.max(img.width,img.height)));cv.width=Math.round(img.width*scale);cv.height=Math.round(img.height*scale);const c=cv.getContext('2d',{willReadFrequently:true});c.imageSmoothingEnabled=true;c.imageSmoothingQuality='high';c.drawImage(img,0,0,cv.width,cv.height);state.thumb=sharpenCanvas(cv,.18);state.cutout=sharpenCanvas(removeEdgeWhite(state.thumb),.26);render();return true}catch(e){console.warn('AI upscale fallback',e);return false}}
