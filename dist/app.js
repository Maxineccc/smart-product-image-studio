...
$('#subjectInput').addEventListener('change',e=>{const file=e.target.files?.[0];if(!file)return;if(state.subjectUrl)URL.revokeObjectURL(state.subjectUrl);const url=URL.createObjectURL(file),img=new Image();img.onload=()=>{state.subjectUrl=url;state.cutout=img;state.thumb=img;state.directSubject=true;state.lastCutoutEngine='original';$('#subjectStatus').textContent=`已载入 ${file.name} · 原图直出，不做画质处理`;render();toast('透明主体已作为原图图层载入')};img.onerror=()=>{URL.revokeObjectURL(url);toast('主体图片读取失败')};img.src=url;e.target.value=''});
