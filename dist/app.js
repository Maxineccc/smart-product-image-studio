...
function drawSubject(centerX,bottomY,maxW,maxH,placeholder){if(!state.cutout){text(state.cutoutBusy?'AI正在生成立体商品主体…':placeholder,centerX,bottomY-maxH/2,18,'600',state.template==='cafe'?'rgba(255,255,255,.62)':'#a0a8b6','center');return}const scale=Number($('#productScale').value)/100,r=Math.min(maxW*scale/state.cutout.width,maxH*scale/state.cutout.height),w=state.cutout.width*r,h=state.cutout.height*r,rotation=Number($('#productRotation').value)*Math.PI/180,opacity=Number($('#productOpacity').value)/100,flip=$('#productFlip').checked?-1:1,shadow=$('#productShadow').checked;if(shadow){ctx.save();ctx.filter='blur(12px)';ctx.fillStyle='rgba(25,18,12,.26)';ctx.beginPath();ctx.ellipse(centerX,bottomY+4,Math.max(38,w*.34),Math.max(7,h*.028),0,0,Math.PI*2);ctx.fill();ctx.restore()}ctx.save();ctx.translate(centerX,bottomY-h/2);ctx.rotate(rotation);ctx.scale(flip,1);ctx.globalAlpha=opacity;if(shadow){ctx.shadowColor='rgba(20,17,12,.28)';ctx.shadowBlur=18;ctx.shadowOffsetY=10}ctx.drawImage(state.cutout,-w/2,-h/2,w,h);ctx.restore()}
function renderSquare(){const v=vals();ctx.save();ctx.fillStyle=$('#whiteBg').checked?'#fff':'#fbfbfb';ctx.fillRect(0,0,800,800);ctx.restore();
  ...
  drawSubject(400+Number($('#productOffsetX').value),700+Number($('#productOffsetY').value),430,600,'上传透明PNG或点击智能解析');
...
function renderCafe(){const v=vals();if(cafeBg.complete&&cafeBg.naturalWidth)ctx.drawImage(cafeBg,0,0,1280,720);else{ctx.fillStyle='#0d4a37';ctx.fillRect(0,0,1280,720)}const brand=(v.brandName||'星巴克').trim();ctx.save();const shade=ctx.createLinearGradient(0,0,720,0);shade.addColorStop(0,'rgba(1,30,21,.2)');shade.addColorStop(.65,'rgba(1,30,21,.08)');shade.addColorStop(1,'rgba(1,30,21,0)');ctx.fillStyle=shade;ctx.fillRect(0,0,720,720);ctx.restore();text('淘宝闪购',52,58,36,'900','#ff5a12');text(`— ${brand}闪购专属价 —`,132,220,32,'650','#fff6dd');text('淘宝闪购',130,320,78,'900','#fff6dd');ctx.save();ctx.fillStyle='#f05212';ctx.beginPath();ctx.moveTo(106,388);ctx.lineTo(565,388);ctx.lineTo(602,438);ctx.lineTo(565,488);ctx.lineTo(106,488);ctx.lineTo(136,438);ctx.closePath();ctx.fill();ctx.restore();text('点外卖更优惠',354,438,48,'900','#fff','center');drawSubject(845+Number($('#productOffsetX').value),665+Number($('#productOffsetY').value),470,610,'上传透明PNG饮料主体')}
function render(){const cafe=state.template==='cafe',w=cafe?1280:800,h=cafe?720:800;if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;canvas.style.aspectRatio=`${w}/${h}`}ctx.clearRect(0,0,w,h);if(cafe)renderCafe();else renderSquare()}
...
['productShadow','productFlip'].forEach(id=>$('#'+id).addEventListener('change',render));
$('#resetPlacement').addEventListener('click',()=>{$('#productScale').value=100;$('#productOffsetX').value=0;$('#productOffsetY').value=0;$('#productRotation').value=0;$('#productOpacity').value=100;$('#productFlip').checked=false;updatePlacementLabels();render();toast('主体已恢复默认位置')});updatePlacementLabels();
function syncTemplate(){state.template=$('#templateSelect').value;const cafe=state.template==='cafe';document.querySelectorAll('.landscape-only').forEach(el=>el.hidden=!cafe);document.querySelectorAll('.square-only').forEach(el=>el.hidden=cafe);$('#templateHint').textContent=cafe?'品牌名称和透明饮料主体均可替换':'适合费用明细与到手价展示';$('#sizeChip').textContent=cafe?'1280 × 720':'800 × 800';$('#exportMeta').textContent=cafe?'1280×720 · 横版JPG':'800×800 · 预计小于400 KB';$('#exportHint').textContent=cafe?'横版咖啡场景，品牌与饮料为独立图层':'JPG，白色补边，不裁切文字';render()}
...
const doubaoAlphaPrompt='只处理我上传图片中的商品主体，严格保持商品的外形、比例、视角、材质、颜色、光影、品牌Logo和所有文字完全不变，不得重绘或修改商品。删除商品以外的全部背景和水印，最终必须导出为真正带 Alpha 透明通道的 PNG：背景像素 Alpha=0，商品边缘保留自然半透明抗锯齿。严禁绘制灰白棋盘格，严禁白底、黑底、彩色底、外框、阴影背景、道具、新增文字或任何水印。请在导出前确认文件包含 RGBA 四通道；如果当前功能无法输出真实 Alpha 透明 PNG，请不要用棋盘格伪装透明，直接回复“当前无法输出真实透明PNG”。';
$('#copyAlphaPrompt').addEventListener('click',async()=>{let ok=true;try{await navigator.clipboard.writeText(doubaoAlphaPrompt)}catch{try{const t=document.createElement('textarea');t.value=doubaoAlphaPrompt;t.style.position='fixed';t.style.opacity='0';document.body.appendChild(t);t.select();ok=document.execCommand('copy');t.remove()}catch{ok=false}}if(!ok)window.prompt('请复制以下豆包透明话术：',doubaoAlphaPrompt);toast(ok?'豆包真实透明话术已复制':'请复制显示的话术')});
$('#subjectInput').addEventListener('change',e=>{const file=e.target.files?.[0];if(!file)return;const url=URL.createObjectURL(file),img=new Image(),status=$('#subjectStatus');img.onload=()=>{if(!hasRealTransparency(img)){URL.revokeObjectURL(url);status.classList.add('warn');status.textContent='这张图片没有透明通道；看到的棋盘格是图片内容，请上传真正透明的PNG/WebP';toast('检测到假透明图片：棋盘格已被画进图片');return}if(state.subjectUrl)URL.revokeObjectURL(state.subjectUrl);status.classList.remove('warn');state.subjectUrl=url;state.cutout=img;state.thumb=img;state.directSubject=true;state.lastCutoutEngine='original';status.textContent=`已载入 ${file.name} · 已确认真实透明通道，原图直出`;render();toast('透明主体已作为原图图层载入')};img.onerror=()=>{URL.revokeObjectURL(url);toast('主体图片读取失败')};img.src=url;e.target.value=''});
...
async function canvasBlob(q){return new Promise(r=>canvas.toBlob(r,'image/jpeg',q))}async function download(){render();let q=.92,blob=await canvasBlob(q);while(blob.size>500*1024&&q>.45){q-=.07;blob=await canvasBlob(q)}state.exportBytes=blob.size;const dims=`${canvas.width}×${canvas.height}`;$('#exportMeta').textContent=`${dims} · ${(blob.size/1024).toFixed(0)} KB`;const base=state.template==='cafe'?($('#brandName').value||'品牌')+'-横版闪购':($('#productName').value||'商品图'),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=base.replace(/[\\/:*?"<>|]/g,'_')+`-${canvas.width}x${canvas.height}.jpg`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),2000);toast('成品图已下载')}
