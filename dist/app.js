...
function renderSocialCoupon(){const v=vals(),brand=(v.brandName||'品牌').trim(),g=ctx.createLinearGradient(0,0,800,800);g.addColorStop(0,'#fffdfa');g.addColorStop(1,'#fff1da');ctx.fillStyle=g;ctx.fillRect(0,0,800,800);drawLogo(44,38,170);text(brand,752,58,23,'800','#20263a','right');text('今天就要喝点好的',400,125,34,'800','#22283b','center');text(v.productName||'产品名称',400,170,20,'650','#767b87','center');fillRound(188,210,424,510,26,'#fff');ctx.save();ctx.shadowColor='rgba(205,125,34,.18)';ctx.shadowBlur=26;ctx.strokeStyle='#f6c77d';ctx.lineWidth=3;roundedRect(ctx,188,210,424,510,26);ctx.stroke();ctx.restore();fillRound(224,239,352,44,22,'#fff1dc');text('淘宝闪购专属优惠',400,261,20,'800','#d89032','center');drawSubject(400+Number($('#productOffsetX').value),500+Number($('#productOffsetY').value),240,260,'上传商品主体');text('外卖叠加红包',400,523,23,'700','#b98a51','center');text('¥'+money(v.finalPrice),400,584,55,'900','#9d5521','center');text('原价 ¥'+money(v.productPrice),400,629,18,'500','#9e8a74','center');fillRound(248,656,304,48,24,'#ff9b42');text('立即使用  ›',400,680,21,'800','#fff','center')}
function renderSocialMinimal(){const v=vals(),brand=(v.brandName||'品牌').trim();ctx.fillStyle='#fff';ctx.fillRect(0,0,800,800);drawLogo(44,38,168);text(brand+' · 今日推荐',752,58,23,'800','#20263a','right');text('领券下单更划算',400,132,32,'850','#fa5b20','center');text(v.productName||'产品名称',400,178,24,'700','#242a38','center');ctx.fillStyle='#fffaf5';ctx.beginPath();ctx.arc(400,420,245,0,Math.PI*2);ctx.fill();drawSubject(400+Number($('#productOffsetX').value),616+Number($('#productOffsetY').value),340,410,'上传商品主体');fillRound(184,652,432,72,12,'#f7f7f8');drawLogo(202,670,112);text('¥'+money(v.finalPrice)+'起',574,679,30,'900','#f2382e','right');text('领取优惠  ›',574,705,17,'650','#6f7787','right');text('淘宝闪购 · 好货即刻到',400,759,17,'500','#9b9faa','center')}
