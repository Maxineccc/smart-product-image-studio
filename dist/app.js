...
function drawWarning(){const v=vals(),w=canvas.width,h=canvas.height,ox=Number($('#warningOffsetX').value),oy=Number($('#warningOffsetY').value),y=h-17+oy,size=w>900?13:w<750?12:11,dark=state.template==='cafe'||state.template==='landscapeDark';ctx.save();ctx.font=`500 ${size}px "Microsoft YaHei","PingFang SC",sans-serif`;ctx.fillStyle=dark?'rgba(255,255,255,.72)':'#737b88';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(v.warningText||'仅限新客使用，适用商品/店铺以活动页面显示为准。',w/2+ox,y,w-50);ctx.restore()}
...
function render(){const def=templateDefs[state.template]||templateDefs.square,w=def.w,h=def.h;if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;canvas.style.aspectRatio=`${w}/${h}`}ctx.clearRect(0,0,w,h);({square:renderSquare,socialCoupon:renderSocialCoupon,socialMinimal:renderSocialMinimal,socialDeal:renderSocialDeal,socialDuo:renderSocialDuo,socialScene:renderSocialScene,squareBestseller:renderSquareBestseller,cafe:renderCafe,landscapeClean:renderLandscapeClean,landscapeDark:renderLandscapeDark}[state.template]||renderSquare)();drawWarning()}
