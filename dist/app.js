...
const doubaoChromaPrompt='只处理我上传图片中的商品主体，严格保持商品外形、比例、视角、材质、颜色、光影、品牌Logo和所有文字完全不变，不得重绘、改字或更换Logo。生成高清电商棚拍商品图，商品完整居中，四周保留适量空间。背景必须是单一、均匀、纯净的品红色 #FF00FF（RGB 255,0,255），从四角到商品边缘颜色完全一致；禁止渐变、纹理、噪点、阴影、光晕、反射、地面、道具、棋盘格、白边、边框、新增文字和视觉水印。商品自身可以保留真实高光与透明材质，但商品外的每一个背景像素都必须为 #FF00FF。输出 1:1 高清 PNG。';
