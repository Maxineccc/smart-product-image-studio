...
$('#openDoubaoBtn').addEventListener('click',async()=>{window.open('https://www.doubao.com/chat/','_blank','noopener,noreferrer');let copied=true;try{await navigator.clipboard.writeText(doubaoGreenPrompt)}catch{try{const t=document.createElement('textarea');t.value=doubaoGreenPrompt;t.style.position='fixed';t.style.opacity='0';document.body.appendChild(t);t.select();copied=document.execCommand('copy');t.remove()}catch{copied=false}}if(!copied)window.prompt('请复制这段提示词后粘贴到豆包：',doubaoGreenPrompt);$('#greenStatus').textContent=copied?'豆包已打开 · 在输入框粘贴提示词并上传原图':'豆包已打开 · 提示词已显示，请复制后粘贴';toast(copied?'豆包已打开，提示词已复制':'豆包已打开，请复制显示的提示词')});
